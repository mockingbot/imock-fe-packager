...
const connectCustomBucket = async ({
  ...
  const getBufferList = async (keyPrefix = '', maxKey) => {
    ...
    const { resultList: [ { relativeFrom, fileList } ] } = await pathAction({ actionType: 'list-file-recursive', key: `${bucket}/${keyPrefix}`, urlPathAction, fileAuth })
  ...
  const getBuffer = async (key) => {
    const buffer = await fileDownload({ filePath: `${bucket}/${key}`, urlFileDownload, fileAuth })
    return { key, buffer }
  }
  const putBuffer = async (key, buffer, accessType = ACCESS_TYPE.PRIVATE) => {
    ...
    await fileUpload({ fileBuffer: buffer, filePath: `${bucket}/${key}`, urlFileUpload, fileAuth })
    return { key }
  }
  const copyBuffer = async (key, sourceInfo, accessType = ACCESS_TYPE.PRIVATE) => {
    ...
    await pathAction({ actionType: 'copy', key: `${bucket}/${sourceInfo.key}`, keyTo: `${bucket}/${key}`, urlPathAction, fileAuth })
  }
  const deleteBuffer = async (key) => {
    await pathAction({ actionType: 'delete', key: `${bucket}/${key}`, urlPathAction, fileAuth })
  }
  const deleteBufferList = async (keyList) => {
    const { errorList } = await catchAsync(pathAction, { actionType: 'delete', nameList: keyList, key: bucket, urlPathAction, fileAuth })

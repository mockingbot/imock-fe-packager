...
const connectCustomBucket = async ({
  ...
  const getBufferList = async (keyPrefix = '', maxKey) => {
    ...
    const { relativePathFrom, fileList } = await clientFileModify({ modifyType: 'list-file-recursive', filePath: `${bucket}/${keyPrefix}`, urlFileModify, fileAuthConfig })
  ...
  const getBuffer = async (key) => {
    const buffer = await clientFileDownload({ filePath: `${bucket}/${key}`, urlFileDownload, fileAuthConfig })
    return { key, buffer }
  }
  const putBuffer = async (key, buffer, accessType = ACCESS_TYPE.PRIVATE) => {
    ...
    await clientFileUpload({ fileBuffer: buffer, filePath: `${bucket}/${key}`, urlFileUpload, fileAuthConfig })
    return { key }
  }
  const copyBuffer = async (key, sourceInfo, accessType = ACCESS_TYPE.PRIVATE) => {
    ...
    await clientFileModify({ modifyType: 'copy', filePath: `${bucket}/${sourceInfo.key}`, filePathTo: `${bucket}/${key}`, urlFileModify, fileAuthConfig })
  }
  const deleteBuffer = async (key) => {
    await clientFileModify({ modifyType: 'delete', filePath: `${bucket}/${key}`, urlFileModify, fileAuthConfig })
  }
  const deleteBufferList = async (keyList) => {
    const errorList = []
    for (const key of keyList) {
      const { error } = await catchAsync(clientFileModify, { modifyType: 'delete', filePath: `${bucket}/${key}`, urlFileModify, fileAuthConfig })
